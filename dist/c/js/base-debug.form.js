define("xg/eid-company/1.0.0/c/js/base-debug.form", ["core-debug"], function(require, exports, module) {
    var core = require("core-debug");
    $("form").on("blur input propertychange change", "*[data-format]", function() {
    ...
    $("form").on("blur", "*[data-format]", function() {
        var patrn = new RegExp($(this).attr("data-format"));
        if (!patrn.exec($(this).val())) {
            if ($(this).val() == "") {
                removeMsgHolder($(this))
    ...
    $("form").on("blur input propertychange change", "*[data-valueas]", function() {
    ...
    $("form").on("focus click", "*[data-onfocus]", function() {
        createMsgHolder("info", $(this).attr("data-onfocus"), $(this))
    });
    $("form").on("input propertychange", "input[name=checkCode]", function() {
        this.value = this.value.toUpperCase()
    });
    $("form").find("input[type=password][check-strength]").on("input propertychange focus", function() {
        $(".strength-bar").remove();
        if (this.value != "") {
