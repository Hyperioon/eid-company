define(function(require, exports, module){
    ...
    var main = {
        ...
        initEvents:function () {
            ...
            self.list.on("click",".reset",function () {
                ...
                $("#modal-confirm .modal-footer a").eq(0).on("click",function () {
                    ...
                    $.ajax({
                        ...
                        cache: false,
                        type: "post",
                        url: '/subShop/user/resetPassword.json',
                        success: function(data){
                            //.log(data);
                            if(data.code==0){
                                tools.tusi("重置密码成功");
            ...
            //禁用/启用账号按钮点击事件
            /*self.list.on("click",".setStatus",function () {
                var customerId = $(this).parent().data("customerid");
                var realname = $(this).parent().data("realname");
                var setStatus = $(this).html();
                $("body").find("#modal-confirm").remove();
                $("body").append(confirmTmp());
                $("#modal-confirm .modal-body").html("你确定要"+setStatus+"<b>"+realname+"</b>的账号吗？");
                $("#modal-confirm").modal("show");
                //确认删除门店按钮点击事件
                $("#modal-confirm .modal-footer a").eq(0).on("click",function () {
                    var data = {
                    ...
                    $.ajax({
                        ...
                        cache: false,
                        type: "post",
                        url: setStatus=="禁用"?'/subShop/user/disabled.json':'/subShop/user/enabled.json',
                        success: function(data){
                            //.log(data);
                            if(data.code==0){
                                tools.tusi("操作成功");
        ...
        submitForm:function (type) {
            ...
            $("#addForm").validate({
                ...
                submitHandler: function(form){   //表单提交句柄,为一回调函数，带一个参数：form
                    $.ajax({
                        type: "post",
                        url: type=="add"?"/subShop/user/add.json":"/subShop/user/edit.json",
